...
Live-tested bug found right after this first shipped: `tool_choice: "auto"` leaves it entirely up to Mistral whether to actually call `search_web`, and NEXUS initially kept falling back to its old pre-search-tool reflex ("I have no live web access") on a clearly time-sensitive question, even with the tool present and working (confirmed separately by calling the Tavily API directly with the same key — real results, 200 OK). The wiring wasn't the problem; the persona prompt wasn't directive enough to overcome the model's trained habit. Fixed by making the prompt imperative rather than merely permissive: when the tool is offered, using it for a time-sensitive question is a MUST, and claiming no live access in that situation is named explicitly as the mistake to avoid.
